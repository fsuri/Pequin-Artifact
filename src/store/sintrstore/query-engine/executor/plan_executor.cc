...
static void InterpretPlan(
  ...
  // NEW: Set the timestamp for the query plan
  executor_tree->SetBasilTimestamp(txn->GetBasilTimestamp());
  ...
  while (status == true) {
    ...
    //std::cerr << "Segfault made it here 4" << std::endl;
    status = executor_tree->Execute();
    //std::cerr << "Segfault made it here 5" << std::endl;
    ...
    /*if (txn->GetBasilTimestamp().getTimestamp() == 5) {
      ...
      std::cerr << "Read set values " << res_key << ". Timestamp is " << time.getTimestamp() << std::endl;
    }*/
    ...
    if (tile.get() != nullptr) {
      ...
      /*
      std::cerr << "Before tile get read set manager" << std::endl;
      ...
      */
      ...
      //std::cerr << "Past for loop" << std::endl;
      //break;
    }
    //std::cerr << "Past if statement" << std::endl;
  }
  ...
  CleanExecutorTree(executor_tree.get());
  //std::cerr << "After cleaning executors" << std::endl;
  plan->ClearParameterValues();
  //std::cerr << "After cleaning parameters" << std::endl;
  on_complete(result, std::move(values));
}
