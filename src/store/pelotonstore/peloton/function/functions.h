...
class PlpgsqlFunctions {
 private:
  static std::unordered_map<oid_t,
                            std::shared_ptr<peloton_peloton::codegen::CodeContext>>
      kFuncMap;

 public:
  static void AddFunction(
      const oid_t oid,
      std::shared_ptr<peloton_peloton::codegen::CodeContext> code_context);

  static std::shared_ptr<peloton_peloton::codegen::CodeContext> GetFuncContextByOid(
      const oid_t oid);
};
