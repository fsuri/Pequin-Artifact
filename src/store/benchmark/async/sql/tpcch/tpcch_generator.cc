...
void GenerateSupplierTable(TableWriter &writer, std::vector<int> &nation_keys) {
  std::vector<std::pair<std::string, std::string>> column_names_and_types;
  ...
  for (int i = 1; i <= tpcch_sql::NUM_SUPPLIERS; i++) {
    std::vector<std::string> values; 
...
int main(int argc, char *argv[]) {
    gflags::SetUsageMessage(
        "generates json file containing SQL tables for TPCCH data\n");
    ...
    GenerateSupplierTable(writer, nation_keys);
