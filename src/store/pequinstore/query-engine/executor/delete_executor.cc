...
bool DeleteExecutor::DExecute() {
  ...
  for (oid_t visible_tuple_id : *source_tile) {
    ...
    // Debug("Tuple at location [%lu:%lu] with commit state: %d.", tuple_location.block, tuple_location.offset,
    ...
    // // if running at snapshot isolation, then we need to retrieve the latest version of this tuple.
    // if (current_txn->GetIsolationLevel() == IsolationLevelType::SNAPSHOT) {
    //   std::cerr << "Taking snapshot branch" << std::endl;
