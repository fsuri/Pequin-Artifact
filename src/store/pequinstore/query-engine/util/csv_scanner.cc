...
CSVScanner::CSVScanner(peloton::type::AbstractPool &pool,
    ...
    : memory_(pool), file_path_(file_path), file_(), buffer_(nullptr),
      buffer_pos_(0), buffer_end_(0), line_(nullptr), line_len_(0),
      line_maxlen_(0), line_number_(0), delimiter_(delimiter), quote_(quote),
      escape_(escape), func_(func), opaque_state_(opaque_state),
      num_cols_(num_cols) {
  // Make column array
