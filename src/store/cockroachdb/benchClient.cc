...
int main(int argc, char **argv) {
  ...
  Client *client;
  enum { MODE_UNKNOWN, MODE_TAPIR, MODE_WEAK, MODE_STRONG } mode = MODE_UNKNOWN;
  ...
  while ((opt = getopt(argc, argv, "c:d:N:l:w:k:f:m:e:s:z:r:")) != -1) {
    switch (opt) {
      ...
      case 'l':  // Length of each transaction (deterministic!)
      {
        ...
        if ((*optarg == '\0') || (*strtolPtr != '\0') || (tLen <= 0)) {
  ...
  while (1) {
    ...
    client->Begin([](int _) {}, []() {}, 69, false);
    gettimeofday(&t1, NULL);

    beginCount++;
    beginLatency +=
        ((t1.tv_sec - t4.tv_sec) * 1000000 + (t1.tv_usec - t4.tv_usec));
    ...
    for (int j = 0; j < tLen; j++) {
      ...
      if (rand() % 100 < wPer) {
        ...
        client->Put(
            key, key,
            [](int, const std::__1::string &, const std::__1::string &) {},
            [](int, const std::__1::string &, const std::__1::string &) {}, 69);
        gettimeofday(&t4, NULL);
    ...
    gettimeofday(&t3, NULL);
    bool status;
    client->Commit([&status](transaction_status_t) { status = true; }, []() {},
                   69);
    gettimeofday(&t2, NULL);

    commitCount++;
    commitLatency +=
        ((t2.tv_sec - t3.tv_sec) * 1000000 + (t2.tv_usec - t3.tv_usec));
