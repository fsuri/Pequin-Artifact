...
void PelotonInit::Initialize() {
  CONNECTION_THREAD_COUNT = settings::SettingsManager::GetInt(
      settings::SettingId::peloton_connection_thread_count);
  ...
  // start GC.
  gc::GCManagerFactory::Configure(settings::SettingsManager::GetInt(settings::SettingId::peloton_gc_num_threads));
  gc::GCManagerFactory::GetInstance().StartGC();

  // start index tuner
  if (settings::SettingsManager::GetBool(settings::SettingId::peloton_index_tuner)) {
  ...
  // start layout tuner
  if (settings::SettingsManager::GetBool(settings::SettingId::peloton_layout_tuner)) {
