...
inline std::string PlanUtil::GetInfo(const planner::AbstractPlan *plan) {
  ...
  if (plan == nullptr) {
    ...
    os << peloton_peloton::GETINFO_SINGLE_LINE << std::endl;
...
inline void PlanUtil::GetInfo(const planner::AbstractPlan *plan,
  ...
  os << StringUtil::Indent(num_indent)
     ...
     << std::endl;
  os << StringUtil::Indent(num_indent + peloton_peloton::ARROW_INDENT)
  ...
  for (auto &child : children) {
    GetInfo(child.get(), os, num_indent + peloton_peloton::ARROW_INDENT);
  }
}
