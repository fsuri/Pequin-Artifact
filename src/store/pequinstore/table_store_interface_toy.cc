...
//TODO: Ideally move all Toy Testing code currently in querysync-server.cc to here.
//TODO: To support that the ToyTableStore will need access to Server Data structures.
...
void ToyTableStore::LoadTable(const std::string &load_statement, const std::string &txn_digest, const Timestamp &ts, const proto::CommittedProof *committedProof){
...
std::string ToyTableStore::ExecReadQuery(const std::string &query_statement, const Timestamp &ts, QueryReadSetMgr &readSetMgr){

    std::cerr << "EXECUTING READ QUERY ON TOY INTERFACE" << std::endl;
    //TODO: Toy Table Store should be constructed with pointers to Server data structures.
...
void ToyTableStore::ExecPointRead(const std::string &query_statement, std::string &enc_primary_key, const Timestamp &ts, proto::Write *write, const proto::CommittedProof *committedProof){
    ...
    // //if(id ==0){  //TODO: ID is not an argument currently.     // Control how many replicas sent prepared values -> if > f+1 -> result should be prepared.
...
void ToyTableStore::ApplyTableWrite(const std::string &table_name, const TableWrite &table_write, const Timestamp &ts, const std::string &txn_digest, 
...
void ToyTableStore::PurgeTableWrite(const std::string &table_name, const TableWrite &table_write, const Timestamp &ts, const std::string &txn_digest){
...
void ToyTableStore::FindSnapshot(std::string &query_statement, const Timestamp &ts, SnapshotManager &ssMgr){

    std::cerr << "FIND SNAPSHOT ON TOY INTERFACE" << std::endl;
}

//Materialize a snapshot on the Table backend and execute on said snapshot.
void ToyTableStore::MaterializeSnapshot(const std::string &query_retry_id, const proto::MergedSnapshot &merged_ss, const std::set<proto::Transaction*> &ss_txns){
   
} 

std::string ToyTableStore::ExecReadOnSnapshot(const std::string &query_retry_id, std::string &query_statement, const Timestamp &ts, QueryReadSetMgr &readSetMgr, bool abort_early ){
