...
void SettingsManager::InitializeCatalog() {
  auto &settings_catalog = peloton_peloton::catalog::SettingsCatalog::GetInstance();
...
const std::string SettingsManager::GetInfo() const {
  ...
  // clang-format off
  info.append(StringUtil::Format("%34s:   %-34i\n", "Port", GetInt(SettingId::peloton_port)));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Socket Family", GetString(SettingId::peloton_socket_family).c_str()));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Statistics", GetInt(SettingId::peloton_stats_mode) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34i\n", "Max Connections", GetInt(SettingId::peloton_max_connections)));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Index Tuner", GetBool(SettingId::peloton_index_tuner) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Layout Tuner", GetBool(SettingId::peloton_layout_tuner) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   (queue size %i, %i threads)\n", "Worker Pool", GetInt(SettingId::peloton_monoqueue_task_queue_size), GetInt(SettingId::peloton_monoqueue_worker_pool_size)));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Parallel Query Execution", GetBool(SettingId::peloton_parallel_execution) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34i\n", "Min. Parallel Table Scan Size", GetInt(SettingId::peloton_min_parallel_table_scan_size)));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Code-generation", GetBool(SettingId::peloton_codegen) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Print IR Statistics", GetBool(SettingId::peloton_print_ir_stats) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34s\n", "Dump IR", GetBool(SettingId::peloton_dump_ir) ? "enabled" : "disabled"));
  info.append(StringUtil::Format("%34s:   %-34i\n", "Optimization Timeout", GetInt(SettingId::peloton_task_execution_timeout)));
  info.append(StringUtil::Format("%34s:   %-34i\n", "Number of GC threads", GetInt(SettingId::peloton_gc_num_threads)));
  // clang-format on
