...
const std::string TileGroupHeader::GetInfo() const {
  ...
  for (oid_t header_itr = 0; header_itr < active_tuple_slots; header_itr++) {
    ...
    // NEXT RANGE
    peloton_peloton::ItemPointer nextPointer = GetNextItemPointer(header_itr);
    os << "Next:[";
    ...
    // PREVIOUS RANGE
    peloton_peloton::ItemPointer prevPointer = GetPrevItemPointer(header_itr);
    os << "Prev:[";
...
void TileGroupHeader::PrintVisibility(txn_id_t txn_id, cid_t at_cid) {
  ...
  os << peloton_peloton::GETINFO_SINGLE_LINE << "\n";
  ...
  for (oid_t header_itr = 0; header_itr < active_tuple_slots; header_itr++) {
    ...
    peloton_peloton::ItemPointer location = GetNextItemPointer(header_itr);
    os << " prev : "
