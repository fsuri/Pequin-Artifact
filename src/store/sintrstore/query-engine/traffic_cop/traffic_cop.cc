...
ResultType TrafficCop::CommitQueryHelper() {
  ...
  if (curr_state.second != ResultType::ABORTED) {
    ...
    // otherwise, rollback
    Panic("Abort should never happen when using Sintrstore Peloton interface");
...
executor::ExecutionResult TrafficCop::ExecuteReadHelper(
    ...
    const sintrstore::snapshot *ss_txns,
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    size_t thread_id,
    bool is_limit) {
...
executor::ExecutionResult TrafficCop::ExecuteWriteHelper(
  ...
  Debug("Txn digest is %s", sintrstore::BytesToHex(*txn_dig, 16).c_str());
  ...
  Debug("IN WRITE HELPER: Txn %s is trying to %s", sintrstore::BytesToHex(*txn_dig, 16).c_str(), commit_or_prepare? "commit" : "prepare");
...
executor::ExecutionResult TrafficCop::ExecutePointReadHelper(
    ...
    sintrstore::proto::Write *write, size_t thread_id, bool is_customer_read) {
  auto &curr_state = GetCurrentTxnState();
...
ResultType TrafficCop::ExecuteReadStatement(
    ...
    SintrMode mode, //mode = 1: eagerRead, mode = 2: readMaterialized, mode = 3: eagerPlusSnapshot, mode = 4: findSnapshot
    sintrstore::QueryReadSetMgr *query_read_set_mgr, //TODO: change to ptr
    sintrstore::SnapshotManager *snapshot_mgr,
    size_t k_prepared_versions,
    const sintrstore::snapshot *ss_txns,
     ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    size_t thread_id)   
{
  UW_ASSERT(1 <= mode <= 4); //Assert we are using a valid Sintr mode
...
ResultType TrafficCop::ExecutePurgeStatement(
  ...
  try {
    ...
    default:
      ...
      Debug("Purge statement. Txn[%s] undo_delete: %d", sintrstore::BytesToHex(*txn_dig, 16).c_str(), undo_delete);
