...
public:
  ...
  void SetCommitProofs(const sintrstore::proto::CommittedProof *proof) {
    commit_proofs_ = proof;
  }

  const sintrstore::proto::CommittedProof *GetCommitProofs() {
  ...
  sintrstore::QueryReadSetMgr* GetReadSetMgr() {
...
private:
   ...
   */
  const sintrstore::proto::CommittedProof *commit_proofs_;
