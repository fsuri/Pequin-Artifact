    ...
    for (oid_t insert_itr = 0; insert_itr < bulk_insert_count; insert_itr++) {
      ...
      auto new_tuple = tuple;
            /*if (trigger_list != nullptr) {
              ...
              if (trigger_list->HasTriggerType(TriggerType::BEFORE_INSERT_ROW)) {
                ...
                trigger_list->ExecTriggers(TriggerType::BEFORE_INSERT_ROW, current_txn, const_cast<storage::Tuple *>(tuple), executor_context_, nullptr, &new_tuple);
      ...
      /*if (trigger_list != nullptr) {
        ...
        if (trigger_list->HasTriggerType(TriggerType::AFTER_INSERT_ROW)) {
          ...
          LOG_TRACE("address of the origin tuple before firing triggers: 0x%lx", long(tuple));
          trigger_list->ExecTriggers(TriggerType::AFTER_INSERT_ROW, current_txn, const_cast<storage::Tuple *>(tuple), executor_context_, nullptr, &new_tuple);
          LOG_TRACE("address of the new tuple after firing triggers: 0x%lx", long(new_tuple));
        }
        if (trigger_list->HasTriggerType(TriggerType::ON_COMMIT_INSERT_ROW)) {
          ...
          LOG_TRACE("address of the origin tuple before firing triggers: 0x%lx", long(tuple));
          trigger_list->ExecTriggers(TriggerType::ON_COMMIT_INSERT_ROW, current_txn, const_cast<storage::Tuple *>(tuple), executor_context_, nullptr, &new_tuple);
          LOG_TRACE("address of the new tuple after firing triggers: 0x%lx", long(new_tuple));
        }
      }*/
    }
